...
Haiway applications are built around **immutable state objects** that serve as data containers:
...
async def main():
    ...
    alice: User = User(id="1", name="Alice", email="alice@example.com")
    
    # Create new instance with updated fields (original unchanged)
    other_alice: User = alice.updated(id="2", email="other_alice@example.com")        
...
- `ctx.scope("app", alice)` creates an execution context containing the `alice` state
- State is **automatically propagated** to all code within the scope (including nested function calls)
- `ctx.state(User)` retrieves the state using its type as a key
- The `.updated()` method creates a new object with modified fields, leaving the original unchanged
